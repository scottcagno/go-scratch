...
func recursiveWalk(n *node, fn WalkFn) bool {
	...
	if n.leaf != nil && fn(n.leaf.key, n.leaf.val) {
		fmt.Printf("[leaf] prefix=%q, key=%q, val=%+v\n", n.prefix, n.leaf.key, n.leaf.val)
		return true
	}
	// Recurse on the children...
	for _, e := range n.edges {
		fmt.Printf("[node] prefix=%q\n", e.node.prefix)
...
// WalkPathAbove recursively walks the tree using the supplied WalkFn fn, under
// a specific prefix supplied by the prefix string.
func (t *Tree) WalkPathAbove(prefix string, fn WalkFn) {
	n := t.root
	search := prefix
	for {
...
// WalkPathBelow recursively walks the tree using the supplied WalkFn fn, under
// a specific path supplied by the path string. It is like WalkPathAbove, but
// instead of visiting all the entries under a given prefix, this walks the
// entries above the path.
func (t *Tree) WalkPathBelow(path string, fn WalkFn) {
	n := t.root
	search := path
	for {
